...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    }, (err) => { if (err.code !== 'permission-denied') console.error("Dashboard error on unsubCampaigns =", err); });
    ...
    }, (err) => { if (err.code !== 'permission-denied') console.error("Dashboard error on unsubNotices =", err); });
    ...
    }, (err) => { if (err.code !== 'permission-denied') console.error("Dashboard error on unsubUsers =", err); });
  ...
  useEffect(() => {
    ...
    }, (err) => { if (err.code !== 'permission-denied') console.error("Dashboard error on unsubWelfare =", err); });
    ...
    const unsubContribs = onSnapshot(qContribs, (snap) => {
      ...
      }, (err) => { if (err.code !== 'permission-denied') console.error("Dashboard error on unsubContribs =", err); });
  ...
  useEffect(() => {
    if (settings?.showTotalBalanceToMembers) {
      ...
      const unsubTotal = onSnapshot(qAllVerified, (snap) => {
        let sum = 0;
        snap.forEach(d => { sum += d.data().amount || 0; }, (err) => { if (err.code !== 'permission-denied') console.error("Dashboard error on unsubTotal =", err); });
        setTotalBalance(sum);
      });
      return () => unsubTotal();
    }
