...
export function BottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-4 py-2 flex justify-between items-center z-20 pb-safe">
      {links.map(({ to, icon: Icon, label }) => {
        ...
        return (
          <Link 
            key={to} 
            to={to} 
            className={`flex flex-col items-center p-2 rounded-lg transition-colors ${isActive ? 'text-mamas-primary' : 'text-slate-400 hover:text-slate-600'}`}
          >
